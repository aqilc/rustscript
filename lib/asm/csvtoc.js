...
let memory_operands = new Set([ "M64", "M32", "M16", "M128", "M256", "FARPTR16" ]);
...
function insert_ins(row, name, operands) {
	...
	if(instruction_variants[insname]) {
		if(instruction_variants[insname][0].replace(/REX\.?\w?\+\s*/, "") !== row[0].replace(/REX\.?\w?\+/, ""))
...
for(let i in instruction_variants) {
	...
	for(let i of opcode) {
		if(!prefdone && const_prefixes[i])
			prefixes += const_prefixes[i] + ", ";
		else if(!prefdone && (i === "PREF.66+" || i === "66"))
			variant += "\n\t\t.pref66 = true,";
		else if (!prefdone && i.startsWith("REX") && !row[15]?.includes("RH") /** RH does not have required rexes */) {
			if(i[3] === "+")
				variant += "\n\t\t.rex = 0x40,";
			else if(i[4] === "W")
				variant += "\n\t\t.rex = 0x48,";
			...
			variant += "\n\t\t.modrm = 0x" + (i[1] === "r" ? "C0" : (0xC0 | (+i[1] << 3)).toString(16)) + ",";
	...
	if(operands?.[0]) {
		if(name !== "ENTER" && name !== "FSTSW" && name !== "FNSTSW" && name !== "LDDQU" && name !== "LEA" && i !== "MOV SREG, R16" &&
			...
			(operands.includes("M16") || operands.includes("R16") || operands.includes("AX") || operands.includes("IMM16"))) variant += "\n\t\t.pref66 = true, ";
	...
	if(row[2].includes("M")) variant += "\n\t\t.mem_operand = " + (row[2].indexOf("M") + 1) + ",";
	if(row[2].includes("R")) variant += "\n\t\t.reg_operand = " + (row[2].indexOf("R") + 1) + ",";
	else if (row[2].includes("O")) variant += "\n\t\t.reg_operand = " + (row[2].indexOf("O") + 1) + ",";
...
// 	// if(row[1].includes("r16") || row[1].includes("m16") || row[1].contains("ax") || row[1].contains("imm16")) {
// 	// 	if()
// 	// 	"ENTER FSTSW FNSTSW LDDQU LEA MOV_SREG_R16 MOVSX_R32_R16 MOVSX_R64_R16 MOVSX_R32_M16 MOVSX_R64_M16 MOVZX_R32_R16 MOVZX_R64_R16 MOVZX_R32_M16 MOVZX_R64_M16 PINSRW RET VERR"
...
#define X64MEM_5_ARGS(disp, base, index, scale, segment) (disp & 0xffffffff\\
	| ((base) > 0xf && (((u64) (base) == 0x20 && (1 << 61)) || (((u64) 1 << 60) | (u64) ((base) & 0xf) << 32))) /*If the operand is more than 32 bits wide or is equal to the RIP register,\\
